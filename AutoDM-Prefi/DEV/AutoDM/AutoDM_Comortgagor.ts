...
test('test', async ({ page }) => {
  await page.goto('https://myapp.dev.rate.com/?invite-guid=745d45df-7e21-4b7a-9598-e7e62e3812f0');
  await page.getByRole('button', { name: 'Accept Cookies' }).click().catch(() => {});
  await page.waitForLoadState('domcontentloaded');

  // Create password only if the form is present (fresh invite)
  ...
  // Sign in only if we're actually on a login page
  const signInHelper = async () => {
    // Wait briefly for any SSO redirect to settle
    await page.waitForTimeout(2000);
    if (!/login\.dev\.rate\.com/i.test(page.url())) return;
