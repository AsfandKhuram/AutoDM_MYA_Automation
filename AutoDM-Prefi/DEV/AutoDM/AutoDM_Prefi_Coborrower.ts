...
test('test', async ({ page, context }) => {
  ...
  // Wait for redirect to my.gr-dev.com
  await page.waitForURL(/my\.gr-dev\.com\/(okta\/oauth\/cb|loans?|loan\/)/i, { timeout: 15000 }).catch(() => {});
  
  // Navigate to loans page if not already there
  if (!/my\.gr-dev\.com\/loans/i.test(page.url())) {
    await page.goto('https://my.gr-dev.com/loans', { waitUntil: 'domcontentloaded' });
  }
  ...
  const ensureLoansContentReady = async () => {
    const loanRow = page.getByText(/(?:purchase|refinance)\s+#?\d+\s*dev/i).first();
  ...
  const purchaseRefiMatches = [...bodyText.matchAll(/(?:purchase|refinance)\s+#?(\d+)\s*dev/ig)];
  ...
  for (const match of purchaseRefiMatches) {
    ...
    if (loanNumeric > latestLoanNumeric) {
      ...
      loanNumber = `${loanDigits}DEV`;
    }
  }

  // Fallback when card text is split unexpectedly: capture any visible ######DEV tokens.
  if (!loanNumber) {
    const devTokenMatches = [...bodyText.matchAll(/\b(\d{6,})\s*dev\b/ig)];
  ...
  await page.goto(`https://myapp.dev.rate.com/apply/confirm-identity?oldLoanNumber=${selectedLoanNumber}`);
  ...
  if (!/coborrower-exists/i.test(page.url())) {
    await page.goto(`https://myapp.dev.rate.com/apply/coborrower-exists?old-loan-number=${selectedLoanNumber}`);
  ...
  await activePage.getByRole('button', { name: 'Skip' }).click({ timeout: 4000 }).catch(() => {});
  await activePage.waitForLoadState('domcontentloaded', { timeout: 2000 }).catch(() => {});
  await activePage.goto('https://my.gr-dev.com/loans');
  ...
  const finalLoanClick = activePage
    .getByText(new RegExp(`(?:purchase|refinance)\\s+#?${selectedLoanNumber.replace(/DEV$/i, '')}(?:dev)?`, 'i'))
    .first();
