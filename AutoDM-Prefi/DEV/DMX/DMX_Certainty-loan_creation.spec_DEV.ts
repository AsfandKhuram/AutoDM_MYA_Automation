...
const loanApplicationUrl = resolveLoanApplicationUrl('certainty');
...
test('test', async ({ page }) => {
  ...
  // Fills the "Declarations" page. The page was redesigned to the heading
  // "Just a few yes or no questions for you" using radiogroups (older variants used Yes/No
  // buttons and a separate /apply/declarations URL — both are still handled here).
  // Per business requirements: occupy primary residence = Yes; prior ownership = Yes
  // (Primary Residence / Sole Ownership); every remaining question = No; Citizenship = US Citizen.
  // No-ops (and does NOT click Continue) if a declarations page is not detected, so it is safe
  // to call once per borrower even if the co-borrower variant skips it.
  const fillDeclarations = async (tag: string) => {
    const onDeclarations = await page.getByRole('heading', { name: /just a few yes or no|^declarations$/i })
      .waitFor({ state: 'visible', timeout: 30000 }).then(() => true).catch(() => false)
      || /\/apply\/declarations/i.test(page.url());
    if (!onDeclarations) {
      _logTime(`${tag}: no declarations page, skipping`);
      return;
    }
    _logTime(`${tag}: declarations page`);

    // Answer the question matching `q`. Supports the new radiogroup layout (named or unnamed)
    // and the legacy Yes/No-button layout.
    const answer = async (q: RegExp, choice: 'Yes' | 'No') => {
      const choiceRe = new RegExp(`^${choice}$`, 'i');

      // New design: radiogroup whose accessible name is the question text.
      const namedGroup = page.getByRole('radiogroup', { name: q });
      if (await namedGroup.first().isVisible({ timeout: 1500 }).catch(() => false)) {
        await namedGroup.first().getByRole('radio', { name: choiceRe }).click({ timeout: 8000 }).catch(() => {});
        return;
      }

      // New design fallback: unnamed radiogroup inside the block that holds the question text.
      const radioBlock = page.locator('div')
        .filter({ hasText: q })
        .filter({ has: page.getByRole('radio') })
        .last();
      if (await radioBlock.getByRole('radio').first().isVisible({ timeout: 1000 }).catch(() => false)) {
        await radioBlock.getByRole('radio', { name: choiceRe }).first().click({ timeout: 8000 }).catch(() => {});
        return;
      }

      // Legacy design: Yes/No buttons.
      const row = page.locator('div')
        .filter({ hasText: q })
        .filter({ has: page.getByRole('button', { name: /^(yes|no)$/i }) })
        .last();
      await row.getByRole('button', { name: choiceRe })
        .click({ timeout: 8000 })
        .catch(() => {});
    };

    // Occupy as primary residence = Yes
    await answer(/occupy the property as your primary residence/i, 'Yes');
    // Prior ownership interest = Yes → reveals the property/title dropdowns
    await answer(/ownership interest in another property in the last three years/i, 'Yes');
    await page.waitForLoadState('networkidle', { timeout: 4000 }).catch(() => {});

    // Conditional dropdowns (only present when prior ownership = Yes)
    const propType = page.getByRole('combobox', { name: /what type of property (did|do) you own/i });
    if (await propType.isVisible({ timeout: 5000 }).catch(() => false)) {
      await propType.selectOption({ label: 'Primary Residence' }).catch(() => {});
    }
    const titleHeld = page.getByRole('combobox', { name: /how (did|do) you hold title/i });
    if (await titleHeld.isVisible({ timeout: 5000 }).catch(() => false)) {
      await titleHeld.selectOption({ label: 'By Yourself' })
        .catch(() => titleHeld.selectOption({ label: 'Sole Ownership' }))
        .catch(() => titleHeld.selectOption({ index: 1 }))
        .catch(() => {});
    }

    // Every remaining declaration question = No
    const noQuestions: RegExp[] = [
      /family relationship or business affiliation with the seller/i,
      /borrowing any money for this real estate transaction/i,
      /applying for a mortgage loan on another property/i,
      /applying for any new credit/i,
      /subject to a lien that could take priority/i,
      /co-?signer or guarantor on any debt/i,
      /outstanding judgments against you/i,
      /delinquent or in default on a Federal debt/i,
      /party to a lawsuit in which you potentially have any personal financial liability/i,
      /conveyed title to any property in lieu of foreclosure/i,
      /completed a pre-?for[e]?closure sale or short sale/i,
      /had (a )?property foreclosed upon in the last 7 years/i,
      /declared bankruptcy within the past 7 years/i,
      /currently pay Alimony/i,
      /currently pay Child Support/i,
    ];
    ...
    const citizenship = page.getByRole('combobox', { name: /citizenship/i });
    if (await citizenship.isVisible({ timeout: 5000 }).catch(() => false)) {
      await citizenship.selectOption({ label: 'US Citizen' })
        .catch(() => citizenship.selectOption({ label: 'US CITIZEN' }))
        .catch(() => citizenship.selectOption({ index: 1 }))
        .catch(() => {});
    }

    await page.waitForLoadState('networkidle', { timeout: 3000 }).catch(() => {});
    await page.getByRole('button', { name: /^continue$/i }).click();
    _logTime(`${tag}: declarations done`);
  };
  ...
  // 23. Declarations page (Andy): redesigned Yes/No-button layout.
  await fillDeclarations('step23');
  _logTime('step23 done (Andy declarations)');
  ...
  // 26. Declarations page (Amy): redesigned Yes/No-button layout.
  await fillDeclarations('step26');
  _logTime('step26 done (Amy declarations)');
  ...
  await page.waitForURL(/login\.dev\.rate\.com|my\.gr-dev\.com\/loan\/|my\.dev\.originpoint\.com|my2\.dev\.rate\.com\/accounts|my\.dev\.certaintyhomelending\.com\/loan\//i);
  ...
  await page.waitForURL(/login\.dev\.rate\.com|my\.gr-dev\.com\/loan\/|my\.dev\.originpoint\.com|my2\.dev\.rate\.com\/accounts|my\.dev\.certaintyhomelending\.com\/loan\//i, { timeout: 5000 }).catch(() => {});
  if (/login\.dev\.rate\.com/.test(page.url())) {
    ...
    await page.waitForURL(/my\.gr-dev\.com\/loan\/|my\.dev\.originpoint\.com\/loan\/|my2\.dev\.rate\.com\/accounts|my\.dev\.certaintyhomelending\.com\/loan\//i);
  ...
  // else fall back to the loan id in URL. No hard assertion here so the captured-data
  // block below always runs even if the banner format changes.
  ...
  if (!fullBannerText && /(my\.gr-dev\.com|my\.dev\.originpoint\.com|my\.dev\.certaintyhomelending\.com)\/loan\/[^/?#]+(\/overview)?$/i.test(page.url())) {
    const overviewUrl = `${page.url().replace(/\/+$/, '').replace(/\/overview$/i, '')}/overview`;
  ...
  const loanNumber =
    ...
    fullBannerText?.match(/loan\s*#?\s*[:\-]?\s*(\d[A-Za-z0-9-]{5,})/i)?.[1] ??
    loanIdFromUrl ??
    fullBannerText?.trim() ??
    '';
