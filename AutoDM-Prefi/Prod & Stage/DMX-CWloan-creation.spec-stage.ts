...
const DEFAULT_LOAN_APPLICATION_URL = 'https://stage-cwhm.dmx.saas.rate.com/?emp-id=36067';
...
test('test', async ({ page }) => {
  ...
  await page.addLocatorHandler(
    ...
    async () => {
      ...
      await acceptCookiesButton.click({ force: true }).catch(() => {});
      await acceptCookiesButton.waitFor({ state: 'hidden', timeout: 3000 }).catch(async () => {
        await page.evaluate(() => {
          document.getElementById('onetrust-banner-sdk')?.remove();
          document.getElementById('onetrust-accept-btn-handler')?.remove();
        }).catch(() => {});
      });
    }
  );
  ...
  // 3. Where in the home buying process? - auto-advances after selection (may be skipped by app)
  if (await page.getByRole('radio', { name: /i.m looking at homes and listings/i }).isVisible({ timeout: 5000 }).catch(() => false)) {
    await page.getByRole('radio', { name: /i.m looking at homes and listings/i }).click();
  }

  // 4. How soon are you looking to buy? - auto-advances after selection (may be skipped by app)
  if (await page.getByRole('radio', { name: /within the next few months/i }).isVisible({ timeout: 5000 }).catch(() => false)) {
    await page.getByRole('radio', { name: /within the next few months/i }).click();
  }

  // 5–9. Welcome flow questions (city, home type, usage, budget, price) — may be skipped in stage
  if (await page.getByRole('textbox', { name: /city or town/i }).isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    if (await page.getByLabel(/i don't know what my monthly budget is yet/i).isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    if (await page.getByRole('textbox', { name: /target.*price|purchase.*price/i }).first().isVisible({ timeout: 5000 }).catch(() => false)) {
  ...
  await page.getByTestId('borrower-marital-status-radio-button-0-Married').click();
  ...
  const myaRedirectPattern = /login\.dev\.rate\.com|my\.gr-dev\.com\/loan\/|my\.rate\.com\/(?:login|loan\/|loans)|my\.onqhomeloans\.com\/(?:login|loan\/|loans)/i;
  ...
  if (/login\.dev\.rate\.com|my\.rate\.com\/login|my\.onqhomeloans\.com\/login/i.test(page.url())) {
    ...
    await page.waitForURL(/my\.gr-dev\.com\/loan\/|my\.rate\.com\/(?:loan\/|loans)|my\.onqhomeloans\.com\/(?:loan\/|loans)/i, { timeout: 60000 });
    await page.waitForLoadState('networkidle').catch(() => {});
  }
  const myaLoansPattern = /my\.gr-dev\.com\/loans|my\.rate\.com\/loans|my\.onqhomeloans\.com\/loans/i;
  const myaLoanOverviewPattern = /my\.gr-dev\.com\/loan\/[^/]+\/overview|my\.rate\.com\/loan\/[^/]+\/overview|my\.onqhomeloans\.com\/loan\/[^/]+\/overview/i;
